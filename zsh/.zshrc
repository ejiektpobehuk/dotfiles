...
__doproxy() {
	doctl compute droplet create ssh-proxy --wait --image ubuntu-18-04-x64 --size 1gb --region fra1 --ssh-keys 23355311 && ssproxy -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -i ~/.ssh/do root@`doctl compute droplet list | grep ssh-proxy | awk '{ print $3 }'`
